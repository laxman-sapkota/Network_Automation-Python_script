...
# Variables
ip_address = "192.168.122.131" # The Default ip Address for router 
user_name = "admin"
pass_word = "cisco"
password_enable = "cisco"  #This is a Password for enable mode
enable = "enable" # Entering Enable command
firstint = "10.0.0.1"  # First interface
secondint = "10.0.0.10"  # Second interface
...
def staticinterfaceone():
    # R1 Setting up interface one
    session.sendline('interface g0/1')
    result = session.expect(['R1\(config-if\)#', pexpect.TIMEOUT])
    ...
    result = session.expect(['R1\(config-if\)#', pexpect.TIMEOUT])
    if result != 0:
                print('Failure! There is a problem with setting the hostname try again')
    else:
...
#    Check for error, if so then print error and exit the program
        ...
        print('Clock rate has set to 64000 for interface gig0/2 as it is a DCE Connection')
        session.sendline('exit')
...
#   Check for error, if so then print error and exit
if result != 0:
        print('Failure! There is a problem creating a session ip address please try again', ip_address)
else:
